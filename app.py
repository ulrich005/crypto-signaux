...
if st.sidebar.button("🔄 Actualiser les signaux"):
    ...
    close_series = df['Close'].squeeze()  # Corrigé ici
    ...
    for i in range(2, len(df)):
        if close_series.iloc[i] > close_series.iloc[i-1] > close_series.iloc[i-2]:
            df.iloc[i, df.columns.get_loc('signal')] = 'Buy'
            buy_prices.append(df['Close'].iloc[i])
        elif close_series.iloc[i] < close_series.iloc[i-1] < close_series.iloc[i-2]:
